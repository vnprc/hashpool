...
This deferred-pay hook is also where block template validation (issue #65) will plug in
later: pay = epoch boundary final AND template valid. One lever, two gates, no rework.
...
(#65 — its result will gate the same pay-at-finality hook built here). Old units simply sit,
