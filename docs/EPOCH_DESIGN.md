...
This deferred-pay hook is also where block template validation will plug in later
([issue #20](https://forge.anarch.diy/vnprc/hashpool/issues/20)): pay = epoch boundary
final AND template valid. One lever, two gates, no rework.
...
([issue #20](https://forge.anarch.diy/vnprc/hashpool/issues/20) — its result will gate the
same pay-at-finality hook built here). Old units simply sit,
