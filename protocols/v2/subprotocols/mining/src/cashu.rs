...
pub fn calculate_work(hash: [u8; 32]) -> u64 {
    calculate_work_with_config(hash, 36, 64)
}

/// Calculate work using exponential valuation (2^n) where n is number of leading zero bits.
/// 
/// This function addresses the fundamental flaw in the linear counting approach:
/// - Each additional zero bit is exponentially more difficult to find
/// - Shares should be valued proportionally to their true computational difficulty  
/// - Compresses representable range from 256 to configurable max_bits
///
/// Parameters:
...
pub fn calculate_work_with_config(hash: [u8; 32], min_leading_zeros: u32, max_representable_bits: u32) -> u64 {
...
mod tests {
    ...
    fn test_calculate_work_expected_values() {
        ...
        assert_eq!(calculate_work_with_config(test_hash, 36, 50), 1u64 << 47);
        
        // Below minimum: should get 0
        let mut low_hash = [0u8; 32];
        low_hash[3] = 1; // 3*8 + 7 = 31 leading zeros, below minimum of 36
        assert_eq!(calculate_work_with_config(low_hash, 36, 50), 0);
    }
    ...
    fn test_exponential_valuation() {
        ...
        hash1[1] = 0x40; // Sets bit 6 of byte 1: 8 + 1 = 9 leading zeros... wait, let me recalculate
        hash1[1] = 0x20; // Sets bit 5 of byte 1: 8 + 2 = 10 leading zeros
        assert_eq!(calculate_work_with_config(hash1, 5, 20), 1u64 << 10);

        // 11 leading zeros: 2^11 = 2048 (double the previous)
        let mut hash2 = [0u8; 32];
        hash2[1] = 0x10; // Sets bit 4 of byte 1: 8 + 3 = 11 leading zeros
        assert_eq!(calculate_work_with_config(hash2, 5, 20), 1u64 << 11);

        // Verify doubling relationship
        assert_eq!(
            calculate_work_with_config(hash2, 5, 20),
            calculate_work_with_config(hash1, 5, 20) * 2
        );
    }
