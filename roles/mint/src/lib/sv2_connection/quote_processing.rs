...
async fn send_quote_response_to_pool(
    ...
    info!("🚀 Sending mint quote response via TCP connection: quote_id={}", quote_id_str);
    
    // Create pool message for the response
    let pool_message = PoolMessages::Minting(
        roles_logic_sv2::parsers::Minting::MintQuoteResponse(response)
    );
...
async fn send_quote_error_to_pool(
    ...
    // Create pool message
    let pool_message = PoolMessages::Minting(
        roles_logic_sv2::parsers::Minting::MintQuoteError(error_response)
    );
